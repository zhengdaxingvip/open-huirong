...
import cn.hrfax.open.util.DateFormatUtils;
...
public class DateUtil {
     ...
     * 判断指定时间是否在规定的时间段内
     * @param nowData
     * @param beginTime 开始时间格式时分（如：7:30）
     * @param endTime 结束时间格式时分（如：19:30）
     * @return
     */
    public static boolean belongCalendar(Date nowData, String beginTime, String endTime) {
        try {
            SimpleDateFormat df = new SimpleDateFormat(hm_DATE_FORMAT);
